...
### 결과 (2026-06-11 실행 — 14 픽스처 × 5 방법, 253 에이전트)

| 레버 | 적중률 | 스왑 | vague | specific | 포스트모템 제외* |
|---|---|---|---|---|---|
| **A 과주장** (감지형) | **85.7%** | 100% | **100%** | 77.8% | 83.3% |
| B 차분 (감지형) | 64.3% | 100% | 60% | 66.7% | 58.3% |
| C 분기 (측정형) | 57.1% | 100% | 40% | 66.7% | 50.0% |
| **D 하중** (측정형) | **78.6%** | 100% | 60% | **88.9%** | 75.0% |
| blindspot (베이스라인) | 50.0% | 100% | 60% | 44.4% | 41.7% |

*Quibi·Juicero(유명 공개 포스트모템)는 전 방법 적중 = 사전지식 오염 의심 — 제외해도 판정 불변.

### 🚪 GATE G0: **통과** (판정 2026-06-11, Fable 5 — 전문: `.argus/eval/G0-verdict.md`)

사전 등록 기준 그대로: C 57.1%·D 78.6% > 베이스라인 50%, 스왑 ≥80% 충족 → **W2.1 진행.**
정직 조항: C의 마진(+1건/14)은 노이즈 — 실질 근거는 D. "vague에서 C 강세" 예측은 반증됨(40%).
스왑 테스트는 전 방법 100%로 변별력 없음(자명 충족). 샷수 교란으로 A의 1위는 할인해 읽되
vague 5/5는 실질 신호. **함의: D=측정 주축, C=질문 원천(적중기 아님), A(기존 시험한다 단계)=
유지·최강 — 적층 원칙 실증.** P0.B(계획/결과 12쌍)는 미실행 → **W2.4 helm 계속 잠금.**
예산 실측: C 1배치 4콜·D 1콜·A 0콜(기존) — 세션 ≤30콜 예산 내. 첫 샘플 <15초는 스트리밍 필수.

---

## P0.B 결과 (2026-06-11 — `p0b-helm-backtest-workflow.js`, 판정 전문 `.argus/eval/P0B-verdict.md`)

계획/결과 12쌍 (diverged 6 / matched 6): **갈라진 계획에서 괴리 선취 C 4/6 · D 5/6 · 합집합 5/6.**
공동 미스 1건(i18n 문자열 누수 — 실행 후에만 드러나는 종류). **그러나 일치한 계획에서 침묵
C 0/6 (평균 갈림 4.3) · D 1/6** — 원시 탐침은 모든 계획에서 말한다.

**🚪 판정: 조건부 통과 — W2.4 helm 활성.** 단 침묵 제약이 활성 조건: ① 무게 게이트(비가역
연산만 자동 제안)가 전부 ② 경량 스캔 기본 출력 = 침묵, 발화는 D 하중이 비가역 문장에 닿을
때만 ③ C 갈림은 풀 탐침 opt-in 전용 ④ 실계획 3건 잔소리-없음 수용 기준이 이 판정의 실측 검증.
