...
## Loop 16 — OPEN 산출 품질 독립 refute: 간판 산출이 자기 규칙을 어긴다 (진짜 결함)

방법론: 해피패스+in-frame 칭찬이 "A급"이라던 OPEN 산출을, 생성을 안 본 독립
적대 심판에게 넘겨 REFUTE(quality-refute.ts). 3개 OPEN 전부 FAIL.

### F-16-1 · P1 · 내용 · world-fact 정직 위반 (프롬프트 자기규칙 line 36 위반)
확인 안 된 세계-사실을 단정형으로: "GTX 역세권 여부에 따라 수급이 크게 달라요",
"입주물량→전세가 하락", "B2B 고객이 떠나는 이유는 가격보다 X가 훨씬 많아요".
프롬프트 line 36이 "~라면/확인하세요 조건부로 쓰라"고 명시한 걸 어김.

### F-16-2 · P1 · 내용 · 무근거 심리·구체 날조 (line 34 "groundless psych=NEVER" 위반)
안 준 심리 추정("FOMO 감각이 있을 수 있는데", "주변 분위기"), 안 준 구체 날조
("온보딩 1~3개월 생산성 0", "2년 약정 15% 할인", "경쟁사 채용공고", "채널 파트너").

### F-16-3 · P2 · 스파인 · lean 누출(일부 논쟁적이나 실재)
"속도가 전략이에요"(판정), "이걸 모르면 도박이에요"(한쪽으로 프레이밍),
next_question subtext가 선택지별 행동방향을 미리 규정. 일부는 정당한 상황-reframe
과 경계 모호 — 전량 위반은 아님(심판 과열 감안).

### 해석 (근원)
STEP-0 라우팅은 견고한데 OPEN *내용*이 샌다. 프롬프트에 규칙이 이미 있는데도
어기는 것 = CLAUDE.md LLM-glue("LLM이 빈틈을 그럴듯하게 채운다")가 간판 산출에서
발생. regex로 못 고침. 후보(창업자 방향 대기): (a) 생성 후 world-fact/날조 스캔
패스(refuter를 하네스가 아니라 파이프라인에 — 단 latency), (b) 산출 문장에
provenance 태깅(확인필요 vs 근거있음)을 UI에서 음영, (c) 프롬프트 재강화(효과
제한적 — 이미 규칙 있음). 이건 깊은 설계결정이라 단독 수정 금지.

### 방법론 성과 (핵심)
"왜 문제가 안 나오나"의 진짜 답: in-frame 칭찬은 검증이 아니다. 독립 refute가
간판 산출의 체계적 결함을 한 번에 노출. quality-refute를 상시 계층으로.
