...
const MCP_GUARD = 'If the Argus tools (argus_predict / argus_resolve / argus_capture) are NOT in your tool list right now, the wire is broken — do not silently skip. Distinguish the two cases before you speak, because the fix differs: (a) OLD VERSION — those names are absent but legacy ones (argus_seal / argus_settle / argus_open_decision) ARE present: the server is connected but stale, because npx reuses a cached install; say so in one brief line in the user\'s language and point at clearing the npx cache plus a session restart, NOT at reconnecting. (b) NOT CONNECTED — neither set is present: say the plugin is active but its MCP server is not connected (/mcp to check, /argus:settings doctor to diagnose). Either way say it ONCE; if you have already said it this session, stay completely silent about it and ignore the rest of this instruction.';
