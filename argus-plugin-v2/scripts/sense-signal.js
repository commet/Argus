...
function buildDiagnosis(preds, candidates) {
  ...
  lines.push(
    '3. LOAD-BEARING ASSUMPTION (the core sense): What single premise must be true for the reasoning in this turn to hold — the one that flips the call if it breaks? A marked conditional ("as long as", "~니까") is the easy 10%; the valuable ones are UNSTATED — surface what the user did not say. If the decision is consequential you MAY name at most ONE, as a bare neutral question in their language, offered as your draft (source "ai_surfaced" via argus_capture) — never a directional statement, never a two-pole fork, never a disclaimed lean.',
  );
