...
function buildDiagnosis(preds, candidates) {
  const lines = [
    ...
    '1. PREDICTION: Does anything imply a future state reality will prove true or false — and roughly when? Judge falsifiability by MEANING ("이 방향이면 나중에 무리가 올 것 같은데" qualifies; a vibe with no checkable state does not). If yes and the call is consequential (real stakes or hard to reverse), you MAY — after fully handling the request — offer ONCE to save it via argus_predict in the user\'s own words (predicate_owner:"user" if they worded it; else confirm_draft:true for one-tap Keep/Reword/Skip). Record exactly ONE falsifiable claim per predicate — if the turn carries two (a ship date AND closing deals, or two numbers), keep only the single most load-bearing one; never conjoin them with "and"/"so" (that stacks two predictions that cannot settle cleanly).',
  ];
  if (preds.length) {
    lines.push(
