...
const schema = JSON.parse(fs.readFileSync(path.join(root, "data", "schemas", "current-course.json"), "utf8"));
...
const cases = [
  {
    ...
    course: {
      ...
      next_step: "Add the middleware regression test, then rerun /argus:verify.",
      prediction_to_check: {
  ...
  },
  {
    ...
    course: {
      ...
      current_course: {
        status: "fork",
        summary: "Keep webapp and plugin as separate surfaces for one release, but align them around Current Course."
      },
      why_this_course: [
        {
      ...
      next_step: "Ship the plugin Current Course pass, then measure repeat use by surface for 30 days.",
      prediction_to_check: {
  ...
  },
  {
    ...
    course: {
      ...
      next_step: "Send the current gap list to EU counsel and resume Argus with the answer.",
  ...
  // a restraint course: no manufactured fork, no fabricated uncertainty, a done-handle.
  {
    ...
    course: {
      ...
      open_risk: null,
      set_aside_options: [],
      next_step: "Nothing else to decide here — name it and keep going.",
      prediction_to_check: null,
      blocked: false,
  ...
  },
  {
    ...
    course: {
      ...
      open_risk: null,
      set_aside_options: [],
      next_step: "You're done — no action needed.",
      prediction_to_check: null,
      blocked: false,
  ...
  },
  {
    ...
    course: {
      ...
      open_risk: null,
      set_aside_options: [],
      next_step: "Keep shipping on it — nothing to change.",
      prediction_to_check: null,
      blocked: false,
...
function validateCourse(testCase) {
  const { id, course } = testCase;
  if (!course) {
    if (testCase.minimalExpected) return;
    fail(id, "missing course");
  }

  for (const field of schema.required) {
    if (!(field in course)) fail(id, `missing required field ${field}`);
  }

  requireString(id, course.label, "label");
  requireString(id, course.current_course?.summary, "current_course.summary");
  if (!schema.properties.current_course.properties.status.enum.includes(course.current_course?.status)) {
    fail(id, `invalid current_course.status ${course.current_course?.status}`);
  }

  if (!Array.isArray(course.why_this_course) || course.why_this_course.length < 1 || course.why_this_course.length > 3) {
    fail(id, "why_this_course must contain 1-3 items");
  }
  for (const [index, reason] of course.why_this_course.entries()) {
    requireString(id, reason.point, `why_this_course[${index}].point`);
  }
  if (testCase.targetRequiresSource && !course.why_this_course.some((reason) => reason.source)) {
  ...
  if (!Array.isArray(course.set_aside_options) || course.set_aside_options.length > 2) {
    fail(id, "set_aside_options must contain 0-2 items");
  }
  for (const [index, road] of course.set_aside_options.entries()) {
    requireString(id, road.option, `set_aside_options[${index}].option`);
    requireString(id, road.why_not_now, `set_aside_options[${index}].why_not_now`);
  }
  ...
  if (testCase.flat) {
    // A flat negative-control decision must NOT manufacture a fork/uncertainty.
    if (course.set_aside_options.length !== 0) {
      fail(id, "flat decision must have an EMPTY set_aside_options — a manufactured alternative is over-fire");
    }
    if (course.open_risk !== null) {
      fail(id, "flat decision must have open_risk: null — manufactured uncertainty is over-fire");
    }
    if (!["proceed", "anchor"].includes(course.current_course.status)) {
      fail(id, `flat decision must use proceed/anchor status, not ${course.current_course.status} (no manufactured fork)`);
    }
  }
  if (course.current_course.status === "fork" && course.set_aside_options.length < 1) {
    // A 'fork' course surfaces the chosen course + at least one real alternative pole.
    fail(id, "a 'fork' status course must show at least one set_aside_options pole (the other viable path)");
  }
  ...
  if (course.current_course.status === "fork" && course.set_aside_options.length >= 1) {
    ...
    const other = (course.set_aside_options[0].option + " " + course.set_aside_options[0].why_not_now).length;
  ...
  requireString(id, course.next_step, "next_step");
  requireString(id, course.detail_path, "detail_path");

  if (course.blocked && !["hold", "revise", "collect_evidence"].includes(course.current_course.status)) {
    fail(id, "blocked courses must use hold, revise, or collect_evidence status");
  }

  if (course.prediction_to_check) {
    for (const field of ["predicate", "check_by", "pass_condition", "fail_condition"]) {
      requireString(id, course.prediction_to_check[field], `prediction_to_check.${field}`);
    }
    const text = `${course.prediction_to_check.predicate} ${course.prediction_to_check.pass_condition} ${course.prediction_to_check.fail_condition}`.toLowerCase();
    if (!text.includes("if") && !text.includes("when") && !text.includes("after")) {
      fail(id, "prediction_to_check must read as a falsifiable future predicate");
    }
  }

  const rendered = renderCourse(course);
  const lines = rendered.split("\n").filter((line) => line.trim() !== "");
  if (lines.length > 16) fail(id, `rendered course is too long (${lines.length} lines)`);
  for (const term of forbiddenSurfaceTerms) {
    if (rendered.toLowerCase().includes(term.toLowerCase())) {
      fail(id, `rendered course leaks machinery term: ${term}`);
    }
  }
}
...
function renderCourse(course) {
  const lines = [
    `## Argus - Current Course - ${course.label}`,
    `Current course: ${course.current_course.summary}`,
    "Why this course:",
    ...course.why_this_course.map((reason) => `- ${reason.point}${reason.source ? ` (${reason.source})` : ""}`)
  ];

  if (course.open_risk) {
    lines.push(`Open risk: ${course.open_risk.issue}`);
    lines.push(`Required check: ${course.open_risk.required_check || course.open_risk.why_it_matters}`);
  }

  for (const road of course.set_aside_options) {
    lines.push(`Set-aside option: ${road.option} - ${road.why_not_now}`);
  }

  lines.push(`Next step: ${course.next_step}`);

  if (course.prediction_to_check) {
    lines.push(`Prediction to check: ${course.prediction_to_check.predicate}`);
    lines.push(`Check by: ${course.prediction_to_check.check_by}`);
  }

  if (course.blocked) lines.push("Status: hold before execution or signoff.");
  lines.push(`Details: ${course.detail_path}`);
  return lines.join("\n");
}

for (const testCase of cases) {
  validateCourse(testCase);
}
