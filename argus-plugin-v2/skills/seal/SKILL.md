---
name: seal
description: "Seal a chosen Argus decision candidate or sail seed into a later-checkable contract. This is the common state transition after either entry point: `/argus:sail` can produce a seed, `/argus:scan` can produce candidates, and `/argus:seal` turns the selected item into a sealed ledger contract. Use when the user says seal this, commit this for later, remember this decision, or chooses a scan candidate id. Invoked as `/argus:seal`."
...
- `/argus:seal` seals one chosen seed or candidate.
- `/argus:settle` later settles all sealed contracts, regardless of source.
...
`argus-watch seal` was the prototype for scan candidates. `/argus:seal` is the
...
- Calling a sealed contract settled before reality is checked by `/argus:settle`.
