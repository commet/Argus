...
4. Run NO ceremony — no `prediction_to_check`, no settlement date, no re-engagement
...
- **`validation`** → do NOT reframe and do NOT re-open. Acknowledge only the
  **decision as already made — never the user's self-assessment.** Firing form
  ...
  that, if it came back wrong, would change their mind. **No prediction to check** (a
...
- **(closed-log) Running a pressure-check, prediction to check, or goal re-framing on a "decided, just logging it / 기록만" input** — acknowledge and stop, unless a genuinely answer-flipping critical check exists (then ONE optional line, never a two-branch pressure-check). [R12 P18]
- **(prediction to check) Appending a `prediction_to_check` / settlement offer as a default closer** — only on an active `open_decision` with a real future checkpoint; never on vent / closed-log / crisis / flat / validation.
