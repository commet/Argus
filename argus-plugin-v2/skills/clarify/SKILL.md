...
> 1c. **BREADTH sweep (R36 — high-stakes / irreversible / multi-domain load_bearing decisions ONLY; SKIP on low-stakes/reversible, where it is ceremony/over-fire).** A head-to-head test (R35) found a single strong pass loses to a multi-agent crew on exactly ONE axis — generation breadth — and the gap is fully captured by three sweeps a single pass usually skips. Run them so the reframe + `hidden_assumptions` carry the crew's value without the crew (this is why the crew is internal-only, not a user surface):
