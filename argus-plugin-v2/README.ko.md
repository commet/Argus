...
| `/argus:chart` | 지금 어디인가? 버전 트리, 미해결 확인, 다음 단계. promote/branch도 여기서. |
...
흡수하지 않는다"). Argus는 조용한 세션 시작 훅을 함께 설치합니다: 현재
프로젝트에 확인일이 지난 계약이 있으면 그 예측을 현실과 대조해보라는
**한 줄**을 출력하고, 그 외에는 아무것도 출력하지 않습니다 — 침묵이
기본값입니다. `/hooks`에서 언제든 끌 수 있습니다.

이게 복리가 붙는 부분입니다: 시간이 지나면 `.argus/`는 무엇을 결정했고,
무엇을 예측했고, 얼마나 맞았는지의 기록이 됩니다.
