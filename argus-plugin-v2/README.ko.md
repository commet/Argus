...
/argus:sail "docs/strategy.md 이 방향이 맞나?"
```

그냥 말하면 됩니다 — 따옴표도 필수 아니고, 외울 문법도 없습니다. 질문에 PR,
이슈, 파일, 브랜치, 문서가 언급되면 Argus가 그 자료를 직접 읽고 작업합니다.
명령어 없이 "이 보고서 임원회의 가져가도 되나?"처럼 말해도 Claude가 알아서
Argus를 호출합니다. 문서 지원: pdf/md/txt는 바로 읽고, **pptx/docx/xlsx/hwpx**
는 외부 도구 없이 텍스트를 추출합니다 (구형 .ppt/.doc/.hwp는 PDF 내보내기를
안내). 산문이 모호할 때를 위한 명시적 `@PR#123` / `@doc:<경로>` 형태는 남아
있습니다.

**설정 제로.** 필요한 모든 것이 플러그인 안에 들어 있습니다. 설치 후 첫
세션에 `/argus:help`를 알려주는 안내가 **딱 한 번** 한 줄 표시됩니다 (그
외엔 침묵). 첫 실행 때 `.argus/config.yaml`이 합리적인 기본값으로 자동
생성됩니다(언어는 자동 감지 — 바꾸거나 다른 이해관계자 페르소나를 고르려면
그 파일을 편집). 세션 기록은 기본적으로 **git-ignore**됩니다 —
[프라이버시](#프라이버시--팀-공유) 참고.
...
| GitHub CLI `gh` | 선택 — 질문에 언급된 PR/이슈 가져오기. 없으면 붙여넣기로 안내 |
