...
뒤에야 답합니다. 그 답이 **Current Course??**(현재 항로) 한 화면입니다:
...
Argus를 호출합니다. 문서 지원: pdf/md/txt는 바로 읽고, **pptx/docx/hwpx**는
외부 도구 없이 텍스트를 추출합니다. 엑셀(xlsx)과 구형 .ppt/.doc/.hwp는
어설프게 추측하는 대신 CSV/PDF 내보내기를 안내합니다. 산문이 모호할 때를
위한 명시적 `@PR#123` / `@doc:<경로>` 형태는 남아 있습니다.
...
| 명령 | 하는 일 |
|---|---|
| `/argus:sail` | **여기서 시작.** 전체 흐름을 돌리고 Current Course??을 렌더링. |
...
```text
clarify ──→ 선원 작업 ──→ verify ──→ 이해관계자 검토 ──→ Current Course??
...
결정에 가까워진 Current Course??은 **계약 씨앗**으로 끝납니다: 확인 날짜가
...
| 경로 | 언제 | 시간 | 출력 토큰 |
|---|---|---|---|
| Minimal | 작고 가역적인 질문 | ~1분 | 적음 |
| 표준 (기본) | 대부분의 결정 | ~4–8분 | ~40–80k |
| Critical | 비가역 / 고영향 | ~8–12분 | ~100–180k |
...
- 비밀로 보이는 것들(`.env*`, `*.key`, private key 블록, 고엔트로피 문자열)은
  diff가 모델로 가거나 디스크에 쓰이기 전에 마스킹하도록 **지시**되어
  있습니다 — 프롬프트 규칙 기반의 완화책이지 기계적 보장이 아니므로, 비밀이
  든 repo에서는 커밋 전 세션 파일을 직접 확인하세요.
...
- 스키마: `data/schemas/*.json` (Current Course??: `current-course.json`,
