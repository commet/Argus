...
| `schemas/current-course.json` | Compressed one-screen user-facing decision-voyage course | Plugin v2.1 Current Course contract |
...
7. **CurrentCourse** — plugin hides the multi-agent machinery in the default `/argus:sail` output. `current_course.json` is the one-screen course users actually consume; deeper artifacts remain available through `/argus:chart`.
