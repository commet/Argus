...
BREADTH (R36 — high-stakes / irreversible / multi-domain OPEN decisions ONLY; SKIP on a low-stakes reversible choice, where it is ceremony/over-fire). A head-to-head test (R35) found a single strong pass loses to a multi-perspective crew on exactly ONE axis — generation breadth — and the gap is fully captured by three sweeps a single pass usually skips. Run them so one screen carries the crew's value without the crew:
