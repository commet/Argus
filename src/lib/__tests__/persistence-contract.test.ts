...
const ROGUE_ALLOWLIST: Record<string, string> = {
  ...
  argus_profile_updated: '스토리지 키 아님 — 정산 후 프로필 갱신 요약 이벤트 (api/mcp/v2). after() 안에서 도는 학습 경로의 유일한 관측점',
  argus_belief_checks: '스토리지 키 아님 — TWIN M5 사전등록 믿음 채점 서버 테이블 (마이그레이션 20260806140000). 등급별 적중률만 내고 확률은 지어내지 않는다',
  argus_delegations: '스토리지 키 아님 — TWIN 범위 위임 서버 테이블 (마이그레이션 20260806120000). 사용자 원문 인용·만료 not null, 성적으로 자동 정지',
  argus_events: '스토리지 키 아님 — 원격 MCP 파일럿 append-only 원장 테이블 (동)',
  argus_returns: '스토리지 키 아님 — 원격 MCP 파일럿 귀환 계약 테이블, 크론이 읽는다 (동)',
  argus_shadow_predictions: '스토리지 키 아님 — TWIN 봉인 예측 서버 테이블 (마이그레이션 20260806080000). service role 전용, 정산 전 공개 금지',
