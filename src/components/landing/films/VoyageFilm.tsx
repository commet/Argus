...
export function VoyageFilm() {
  ...
  // WCAG 2.2.2 (Pause, Stop, Hide): the film is a ~40s auto-playing, looping
  // moving image. Under prefers-reduced-motion we must NOT start it automatically.
  // `useReducedMotion` resolves on the client (can flip false→true after the
  // autoPlay attribute already kicked the video off), so we also pause + rewind
  // to the poster frame here; native `controls` (set below when rm) then give the
  // user an explicit opt-in to play it. The film's own pixels/timing are untouched.
  useEffect(() => {
    const v = vref.current;
    if (!v || !rm) return;
    v.pause();
    try { v.currentTime = 0; } catch { /* not yet seekable; poster still shows */ }
  }, [rm]);
  ...
  return (
    <figure className="relative w-full h-full" style={{ margin: 0, overflow: 'hidden', background: 'var(--bp-paper)' }}>
      <video
        ...
        autoPlay={!rm} muted loop={!rm} controls={rm} playsInline preload="metadata"
