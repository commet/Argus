 ...
 * 밴드만 손잡이(회항 버튼)로 접힌다.
 *
 * 주: 정적 렌더는 기본 EN 로케일 — 그룹명은 Frame/Draft/Check, 질문은 Q&A로
 * 나온다. 노드 라벨(상황·처음 생각·초안·봉인)은 cps가 준 문자열이라 로케일 불변.
 */
...
const cps: RailCheckpoint[] = [
  ...
  { key: 'draft', label: '초안', state: 'future', group: '작성', groupEn: 'Draft' },
...
describe('CheckpointRail — 3밴드 정거장 상태바', () => {
  ...
  it('세 그룹(Frame/Draft/Check)이 밴드로 보인다', () => {
    for (const group of ['Frame', 'Draft', 'Check']) {
  ...
  it('미래 밴드(Draft/Check)는 실단계를 미리보기로 접어 보여준다', () => {
  ...
  it('지나온 손잡이만 회항 버튼이다 — 미래는 가짜 어포던스 없음', () => {
    ...
    expect(html).not.toMatch(/Look back at Draft|Look back at Check/);
  });
