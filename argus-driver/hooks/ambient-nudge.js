...
function main(input) {
  ...
  return [
    '[Argus ambient — 이번 턴의 대기 시간용 선택 지침. 사용자에게는 그의 언어로 말하라.]',
    `${counts}이(가) 확인일에 도달해 있습니다.`,
    '이번 턴이 오래 걸리는 단계(백그라운드 빌드·테스트·서브에이전트 등)를 포함하거나, 사용자의 요청을 마치고 여유가 남을 때만 — 정확히 한 번 — argus_check_in을 호출해 첫 due 항목 하나를 사용자에게 물어보라. 규칙:',
    '- 사용자의 원래 요청이 항상 먼저다. 요청 처리를 지연시키며 묻지 마라. 빠르게 끝나는 턴이면 완전히 침묵하라.',
