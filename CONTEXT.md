...
```
항로 재설정 → 선원 배치 → 리허설 → 항로 수정 → 이타카(산출물)
```

| 단계 | 항해 비유 | 업무명 | 기능 | 코드 |
|------|-------------|--------|------|------|
| 1 | **항로 재설정** | 문제 재정의 | 과제의 진짜 질문 찾기, 가설 수립, 전제 점검 | DecomposeStep.tsx |
| 2 | **선원 배치** | 실행 설계 | Governing idea + SCR 스토리라인 + 워크플로우 + 판단 포인트 + 핵심 가정 | RecastStep.tsx |
| 3 | **리허설** | 사전 검증 | 페르소나 기반 프리모템 + 리스크 3분류 + 가정 공격 + 승인 조건 | PersonaFeedbackStep.tsx |
| 4 | **항로 수정** | 피드백 반영 | 리허설 피드백 반영 → 재검증 → 항로 수렴 | RefinementLoopStep.tsx |
| → | **이타카** | 산출물 | 4종 Output 생성 | OutputSelector.tsx |

**종합(Synthesize)**: 메인 플로우에서 제외됨. 여러 AI 결과물을 비교할 때 독립적으로 쓰는 유틸리티. 사이드바에서도 제거됨.

---

## 3. Argus Method (지적 자산)
...
항로 재설정의 가설 → 선원 배치의 governing_idea → 리허설에서 프리모템 → 항로 수정에서 더블루프 체크
```
...
| 비유명 | 영문명 | 설명 | 파일 |
|-------|--------|------|------|
| 항해일지 | Project Brief | 경영진/팀에 공유하는 의사결정 기록 | project-brief.ts |
| 선원 지시서 | Prompt Chain | AI에 순서대로 입력할 프롬프트 세트 | prompt-chain.ts |
| 전체 해도 | Agent Spec | LangGraph/CrewAI 구현 출발점 (YAML) | agent-spec.ts |
| 점검표 | Execution Checklist | 순서대로 체크하며 실행하는 리스트 | checklist.ts |
...
- 선원 배치 단계 maxTokens: 3500 (가장 큰 출력)
...
### 항해 메타포 적용 위치
- 로딩 메시지: "항로를 그리고 있습니다...", "선원을 배치하고 있습니다..."
- 빈 상태: "아직 출항하지 않았습니다", "갑판에 선원이 없습니다"
- 수렴 지표: "항로 지수", "항로 완성"
- 다음 단계: "항로가 잡혔습니다 → 선원이 배치되었습니다 → 첫 선원 회의가 끝났습니다"
- Output 네이밍: 항해일지 / 선원 지시서 / 전체 해도 / 점검표
...
### Workflow Graph
- WorkflowGraph.tsx: 선원 배치 단계의 시각적 워크플로우 그래프
- 카드 기반 StepEntry UX
...
2. 전환: 전략기획에서 답을 찾았다
3. 세 장면: 질문을 만드는 사람 (항로 재설정) → 서로 다른 언어를 엮는 사람 (리허설) → 실행을 설계하는 사람 (선원 배치)
4. 브릿지: AI 오케스트레이션과의 연결 (항로 재설정, 리허설, 선원 배치 콜백)
5. Argus 소개 + 4단계
6. 결론: "만들 것을 결정하는 것은 여전히 사람의 몫입니다. Argus는 그 결정이 시작되는 첫 항로입니다."

### 4단계 워딩 (글에서)
```
항로 재설정 — 주어진 과제 뒤에 숨은 진짜 질문을 찾아냅니다.
선원 배치 — 누가 어떤 역할을 맡고 어떤 순서로 움직일지 설계합니다.
리허설 — 이해관계자 앞에서 한 번 띄워봅니다.
항로 수정 — 맞을 때까지 반복합니다.
```
...
- **pm-skills** (github.com/phuryn/pm-skills): 65개 PM 스킬 모음. Argus와는 다른 접근 (도구 모음 vs 사고 파이프라인). 프리모템 분류 체계와 가정 4축을 참고하여 Argus에 적용.
...
- [ ] 선원 배치 단계 UI에 새 필드 표시 (governing_idea, storyline, expected_output, judgment, key_assumptions 등) — 일부 다른 세션에서 작업됨
...
│   │   └── OutputSelector.tsx    # 이타카 — 4종 산출물
│   └── workspace/
│       ├── DecomposeStep.tsx     # 항로 재설정
│       ├── RecastStep.tsx   # 선원 배치
│       ├── PersonaFeedbackStep.tsx # 리허설
│       ├── RefinementLoopStep.tsx  # 항로 수정
│       ├── SynthesizeStep.tsx    # 종합 (메인 플로우 외)
│       └── WorkflowGraph.tsx     # 선원 배치 시각화
├── lib/
