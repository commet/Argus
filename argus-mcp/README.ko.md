...
Argus는 무게 있는 판단과 그 판단이 기대던 전제, 그리고 나중에 현실이 어떻게
됐는지를 보존합니다. 사람을 채점하거나 평결을 내리지 않습니다.
...
평소처럼 말하면 됩니다. Argus는 하나만 남기자고 제안하고, 나중에 돌아옵니다.
...
`ARGUS_DIR`는 터미널 호스트보다 여기서 더 중요합니다. 데스크탑 앱에는 "현재
프로젝트"라는 게 없어서, 이걸 안 주면 서버가 어디에 써야 할지 알 수 없습니다.
절대 경로로 주세요. Windows에서는 역슬래시를 두 번 씁니다
...
프로젝트마다 원장 하나가 내 디스크에, 첫 호출부터 — 따로 준비할 게 없습니다.

MCP 호스트가 프로젝트 디렉터리에서 서버를 띄운다면 `ARGUS_DIR`는 선택입니다.
기본값은 `<현재-프로젝트>/.argus`이고, 호출마다 넘기는 절대 경로 `argus_dir`가
둘 다를 덮어씁니다.
