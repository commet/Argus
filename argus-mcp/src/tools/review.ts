  ...
  handler: async (a) => {
    try {
      ...
      return envelope({
        ...
        next_actions: ['argus_save_prediction', 'skip'],
        data: {
          ...
          protocol: ko ? [
            ...
            '4) 현실이 pass/fail로 답할 반증 가능한 예측 1개를 찾고, 사용자가 원하면 argus_save_prediction으로 저장한다. 예측·pass/fail 조건·check_by는 사용자의 것이다.',
          ] : [
            ...
            '4) Find one falsifiable prediction that reality can answer pass/fail. Save it with argus_save_prediction only if the user wants; the prediction, conditions, and check_by belong to the user.',
          ],
        },
      });
    } catch (e) {
