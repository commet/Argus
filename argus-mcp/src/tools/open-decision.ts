...
const inputSchema = {
  ...
  properties: {
    argus_dir: { type: 'string', description: 'Absolute path to the .argus directory. No "..".' },
    id: { type: 'string', minLength: 1, maxLength: 128, pattern: '^[A-Za-z0-9._-]+$', description: 'Single identifier for this decision (used as the file segment). A new decision gets a new id.' },
    decision: { type: 'string', minLength: 1, maxLength: 600, description: 'The choice the user actually faces, in one neutral sentence. A choice, not an opinion.' },
    stakes: { type: 'string', enum: ['trivial', 'low', 'moderate', 'high'], description: 'Cost of being wrong. If between two, pick the lower (restraint default).' },
    reversibility: { type: 'string', enum: ['one_way_door', 'costly_to_reverse', 'easily_reversible'] },
    status_quo: { type: 'string', minLength: 1, maxLength: 300, description: 'What happens if nothing is done — so "leave_as_is" is always a real option.' },
    already_decided: { type: 'boolean', default: false },
    user_question: { type: 'string', maxLength: 600 },
    crux_question: { type: 'string', maxLength: 400, description: 'The ONE neutral load-bearing question, phrased as a question. Never a fork, never a lean.' },
    load_bearing_assumption: { type: 'string', maxLength: 400, description: 'The single assumption the decision rests on (neutral).' },
    related_to: { type: 'array', items: { type: 'string', pattern: '^[A-Za-z0-9._-]+$' }, maxItems: 20, description: 'Ids of past decisions the user considers similar — surfaces a frequency-only track record, never a verdict.' },
