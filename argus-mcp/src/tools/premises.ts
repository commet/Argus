    ...
    events.length === 0
      ...
      : (events.length === 1
          ? (ko
              ...
              : `Noted: "${oneLine(echo[0]?.text ?? '')}". Fix anything wrong with argus_clarify_decision.${monitoredNote}`)
          : (ko
              ? `전제 ${events.length}건을 기록했습니다 (${refRange}). 틀린 것이 있으면 op=amend로 고치세요. 고친 내용도 기록에 남습니다.${monitoredNote}`
              : `${events.length} premise(s) recorded (${refRange}). Fix anything wrong with argus_clarify_decision; your correction stays on the record too.${monitoredNote}`));

  return envelope({
    ok: true, tool: 'argus_premises',
    surface,
    next_actions: ['argus_save_prediction', 'argus_history', 'leave_as_is'],
...
async function opAmend(
  ...
  if (typeof ref !== 'string' || !action) {
    return toolError({ ok: false, tool: 'argus_premises', error_code: 'AMEND_NEEDS_REF', message: 'op=amend needs `ref` (e.g. "P1") and `action`.', recovery: 'List premises via argus_history view="decision_context", then amend by ordinal.' });
  }
  ...
  return envelope({
    ok: true, tool: 'argus_premises',
    surface,
    next_actions: ['argus_history', 'leave_as_is'],
    data: {
...
async function opResolve(
  ...
  if (typeof ref !== 'string') {
    return toolError({ ok: false, tool: 'argus_premises', error_code: 'RESOLVE_NEEDS_REF', message: 'op=resolve needs `ref`.', recovery: 'List open questions via argus_history view="decision_context", then resolve by ordinal.' });
  }
  const premise = resolvePremiseRef(existing, ref);
  if (premise.kind !== 'open_question') {
    return toolError({ ok: false, tool: 'argus_premises', error_code: 'NOT_AN_OPEN_QUESTION', message: `P${premise.ordinal} is a premise, not an open question.`, recovery: 'Re-check premises with argus_clarify_decision action="update_fact". Only an open question takes the user\'s closing call.' });
  }
  if (premise.status !== 'active') {
    return toolError({ ok: false, tool: 'argus_premises', error_code: 'PREMISE_RETIRED', message: `P${premise.ordinal} is already ${premise.status}.`, recovery: 'Read it via argus_history view="decision_context".' });
  }
...
async function opStillOpen(
  ...
  const ref = a['ref'];
  if (typeof ref !== 'string') {
    return toolError({ ok: false, tool: 'argus_premises', error_code: 'STILL_OPEN_NEEDS_REF', message: 'op=still_open needs `ref`.', recovery: 'List open questions via argus_history view="decision_context", then defer by ordinal.' });
  }
  const premise = resolvePremiseRef(existing, ref);
  if (premise.kind !== 'open_question') {
    return toolError({ ok: false, tool: 'argus_premises', error_code: 'NOT_AN_OPEN_QUESTION', message: `P${premise.ordinal} is a premise, not an open question.`, recovery: 'Only an open question can be left open. Re-check a premise with argus_clarify_decision action="update_fact".' });
  }
  if (premise.status !== 'active') {
    return toolError({ ok: false, tool: 'argus_premises', error_code: 'PREMISE_RETIRED', message: `P${premise.ordinal} is already ${premise.status}.`, recovery: 'Read it via argus_history view="decision_context".' });
  }
