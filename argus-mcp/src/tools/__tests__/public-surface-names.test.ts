...
describe('호스트-대면 표면에 내부 이름 누수 없음', () => {
  ...
  it('공개 표면은 정확히 목적별 7개 이름이다 (내부 15개와 분리)', () => {
    expect([...PUBLIC].sort()).toEqual([
      'argus_check_in', 'argus_clarify_decision', 'argus_history', 'argus_record_result',
      'argus_review_document', 'argus_save_prediction', 'argus_settings',
    ]);
  });
});
