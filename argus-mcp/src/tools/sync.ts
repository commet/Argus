  ...
  handler: async (a) => {
    try {
      ...
      return envelope({
        ...
        next_actions: localSettleableDueCount > 0 ? ['argus_record_result', 'stop'] : ['stop'],
        data: {
          ...
          receipts: receipts.map((r) => {
            ...
            return {
              id: r.id,
              local_id: localId,
              settle_path: localId ? 'argus_record_result (use local_id)' : otherLedger ? 'another terminal ledger (or webapp)' : 'webapp',
