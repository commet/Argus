...
const inputSchema = {
  ...
  properties: {
    ...
    outcome: { type: 'string', enum: ['held', 'avoided', 'partial', 'still_pending'], description: "What reality did to the prediction. Record the user's words — never infer." },
...
export const settle: ToolModule = {
  ...
  handler: async (a) => {
    try {
      ...
      const outcome = a['outcome'] as 'held' | 'avoided' | 'partial' | 'still_pending';
