...
const inputSchema = z.strictObject({
  ...
  predicate: z.string().min(8).max(400).describe('ONE prediction reality can mark true/false. Good: "cutover downtime < 5 min". Bad: "it will go well".\n\nOne claim per call. When the user expects several things at once ("mostly smooth, a couple of edge cases, done by Friday"), that is three predictions, not one — seal them as separate calls so each settles on its own. Do not bundle them into one sentence: a bundle cannot be marked true or false, which is the whole point of sealing it. Do not silently drop the rest either — name the ones you are not sealing yet and offer them.'),
  check_by: zDate.describe('YYYY-MM-DD, a real future date when the result can be checked. Compute it from the CURRENT date, not from your training-time sense of "now" — a date in the past is rejected, and that is the most common first-call failure. If the user said "in two weeks", resolve that against today.'),
