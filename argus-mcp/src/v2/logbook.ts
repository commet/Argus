...
export function renderLogbook(brief: BriefState, repositoryId: string): string {
  ...
  if (brief.due.length > 0) {
    ...
    out.push('');
    out.push('→ `argus_resolve`로 실제 결과를 기록하세요. 아직 결과가 없으면 다음 확인일로 미룰 수 있습니다.');
    out.push('');
  } else {
    out.push('## 결과를 확인할 예측');
    out.push('');
  ...
  if (brief.unsealed_net.length > 0) {
    ...
    for (const n of brief.unsealed_net) {
      out.push(`- ${line(n.text)} · ${n.harvested_on} 포착 (남기려면 \`argus_predict\`)`);
    }
    out.push('');
  }
  ...
  if (brief.premise_rechecks_due.length > 0) {
    ...
    for (const p of brief.premise_rechecks_due) {
      out.push(`- ${line(p.text)} · ${p.due_since}부터 (\`argus_capture\`로 사실 갱신)`);
    }
    out.push('');
  }
