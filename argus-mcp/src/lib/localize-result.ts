...
const KO_ERRORS: Record<string, ErrorCopy> = {
  ...
  ALREADY_CLOSED: { message: '이미 진행 중이거나 닫힌 결정입니다.', recovery: '실제 결과를 기록하려면 argus_record_result를 사용하세요. 닫힌 결정은 다시 열지 않습니다.' },
  ...
  AMEND_NEEDS_REF: { message: '수정할 전제 번호와 작업이 필요합니다.', recovery: 'argus_history view="decision_context"로 목록을 확인한 뒤 ref와 action을 전달하세요.' },
  ...
  RESOLVE_NEEDS_REF: { message: '답을 닫을 미결 질문 번호가 필요합니다.', recovery: 'argus_history view="decision_context"로 목록을 확인한 뒤 ref를 전달하세요.' },
  STILL_OPEN_NEEDS_REF: { message: '열어둘 미결 질문 번호가 필요합니다.', recovery: 'argus_history view="decision_context"로 목록을 확인한 뒤 ref를 전달하세요.' },
  NOT_AN_OPEN_QUESTION: { message: '이 항목은 미결 질문이 아니라 전제입니다.', recovery: '전제는 argus_clarify_decision action="update_fact"로 현실과 다시 확인하고, 미결 질문만 사용자의 말로 닫으세요.' },
  ...
  RECEIPT_NOT_FOUND: { message: '해당 결정의 판단 영수증을 찾지 못했습니다.', recovery: 'argus_history view="all"에서 id를 확인하거나 먼저 예측을 저장하세요.' },
  PREMISES_NEEDS_ID: { message: '전제를 불러올 결정 id가 필요합니다.', recovery: '결정 id를 전달하세요.' },
  NOT_RECHECKABLE: { message: '이 항목은 현실과 재확인할 전제가 아니라 사용자가 답할 미결 질문입니다.', recovery: 'argus_clarify_decision action="answer_question"에 사용자의 판단을 그대로 전달하세요.' },
  ...
  DEFER_DATE_REQUIRED: { message: '다시 확인할 날짜가 필요합니다.', recovery: '사용자에게 날짜를 물어 defer_to에 YYYY-MM-DD로 전달하세요. 더는 중요하지 않다면 argus_clarify_decision action="close"를 사용하세요.' },
