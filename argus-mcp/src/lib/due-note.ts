...
export function appendDueNote(
  ...
  try {
    ...
    const parts: string[] = [];
    if (due.premiseFactsDue > 0) parts.push(`${due.premiseFactsDue} premise fact(s) to re-check (argus_clarify_decision action=update_fact)`);
    if (due.openQuestionsDue > 0) parts.push(`${due.openQuestionsDue} open question(s) to reconsider (argus_clarify_decision)`);
    if (due.contractsDue > 0) parts.push(`${due.contractsDue} prediction result(s) to record (argus_record_result)`);
