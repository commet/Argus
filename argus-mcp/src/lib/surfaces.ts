...
export const SURFACES: Record<SurfaceLocale, SurfaceStrings> = {
  en: {
    ...
    checkin: {
      ...
      due_contracts: (n) => `${n} saved prediction(s) past check-by. Time to record what happened (argus_record_result).`,
      anchor_mirror: (days, n, words) =>
        `${days} day(s) since you saved this, and ${n} prediction(s) are past check-by. Back then you wrote: '${words}' All that's left is to record what actually happened (argus_record_result).`,
      due_premises: (n, staleDays, sinceAdd) =>
        `${n} premise fact(s) due for a re-check${n === 1 && staleDays != null ? ` (last checked ${staleDays}d ago)` : n === 1 && staleDays === null && sinceAdd != null ? ` (added ${sinceAdd}d ago, first check still open)` : ''} (argus_clarify_decision action=update_fact).`,
      reconsider_one: (days, q) =>
        `${days === null ? 'You left this open a while ago' : `You left this open ${days} day(s) ago`}: '${q}' Answer it now if you can, or leave it open a while longer. Either is fine (argus_clarify_decision).`,
      reconsider_more: (n) => `${n} open question(s) you left unresolved are up for another look (argus_clarify_decision).`,
      dropped_lines: (n) =>
    ...
    sync: {
      live_with_due: (total, due) =>
        ...
        'For terminal-saved predictions, record the result here with argus_record_result and local_id. For web-saved predictions, use the web dashboard.',
      live_no_due: (total) => `${total} live prediction(s) in your account. Nothing past its check-by.`,
      settled_on_web: (n) => ` ${n} result(s) already recorded on the web. Use argus_settings action=sync with import_settlements:true to mirror them here, or record them with argus_record_result.`,
    ...
    wake: {
      ...
      overdue_hint: '← argus_record_result',
    ...
    receipt: {
      ...
      premises_note: (tracked, changed) =>
        `(+${tracked} premise(s) tracked · ${changed} changed at re-check · argus_history view=decision_context)`,
    ...
    tools: {
      ...
      seal: {
        sealed: (predicate, checkBy) => `Prediction saved. "${predicate}" Check-by is ${checkBy}. Come back then with argus_record_result to record what happened.`,
      ...
      settle: {
        settled: (outcome) => `Result recorded: ${outcome}. The receipt keeps your prediction beside what actually happened. No grade. (argus_history view=receipt)`,
      ...
      candidates: {
        ...
        promoted: (candidateId, decisionId) => `Linked candidate ${candidateId} to decision ${decisionId}. To make it a tracked prediction, save it with argus_save_prediction.`,
      ...
      watch: {
        ...
        captured: (kind) => `Captured (${kind}). It sits on the internal watch log. Adding it to a decision is your call (argus_clarify_decision).`,
  ...
  ko: {
    ...
    checkin: {
      ...
      due_contracts: (n) => `저장한 예측 ${n}건이 확인일을 지났습니다. 실제 결과를 기록할 차례입니다 (argus_record_result).`,
      anchor_mirror: (days, n, words) =>
        `예측을 저장한 지 ${days}일이 지났고, ${n}건이 확인일을 넘겼습니다. 그때 당신은 이렇게 적었습니다: '${words}' 실제로 어떻게 됐는지만 기록하면 됩니다 (argus_record_result).`,
      due_premises: (n, staleDays, sinceAdd) =>
        `전제 사실 ${n}건을 다시 확인할 차례입니다${n === 1 && staleDays != null ? ` (마지막 확인 후 ${staleDays}일)` : n === 1 && staleDays === null && sinceAdd != null ? ` (적어둔 지 ${sinceAdd}일, 아직 첫 확인 전)` : ''} (argus_clarify_decision action=update_fact).`,
      reconsider_one: (days, q) =>
        `${days === null ? '얼마 전' : `${days}일 전`}에 미결로 남겨둔 질문입니다: '${q}' 지금 답할 수 있으면 답하고, 그대로 열어두어도 괜찮습니다 (argus_clarify_decision).`,
      reconsider_more: (n) => `미결로 남겨둔 질문 ${n}건을 다시 볼 차례입니다 (argus_clarify_decision).`,
      dropped_lines: (n) =>
    ...
    sync: {
      live_with_due: (total, due) =>
        ...
        '터미널에서 저장한 예측은 local_id와 argus_record_result로 결과를 기록하고, 웹에서 저장한 예측은 웹 대시보드에서 기록하세요.',
      live_no_due: (total) => `계정에 살아 있는 예측 ${total}개. 확인할 차례가 된 것은 없습니다.`,
      settled_on_web: (n) => ` 웹에서 이미 결과를 기록한 예측이 ${n}건 있습니다. argus_settings action=sync에 import_settlements:true를 주면 웹 기록을 이 원장으로 옮겨옵니다 (argus_record_result로 직접 적어도 됩니다).`,
    ...
    receipt: {
      ...
      skipped: '— (없음)',
      premises_note: (tracked, changed) =>
        `(추적한 전제 ${tracked}건 · 재확인에서 바뀐 것 ${changed}건 · argus_history view=decision_context)`,
      deferred_fact: (times, originallyDue) =>
    ...
    tools: {
      ...
      seal: {
        sealed: (predicate, checkBy) => `예측을 저장했습니다. "${predicate}" 확인일은 ${checkBy}입니다. 그날 argus_record_result로 실제로 어땠는지 기록하세요.`,
      ...
      settle: {
        settled: (outcome) => `실제 결과를 기록했습니다: ${({ held: '그렇게 됨', avoided: '피함', partial: '부분', missed: '빗나감' })[outcome]}. 영수증에 예측과 실제가 나란히 남습니다. 평가는 없습니다. (argus_history view=receipt)`,
      ...
      candidates: {
        ...
        item: (id, kind, grade, quote) => `- ${id} (${kind}, ${grade}): ${quote}`,
        promoted: (candidateId, decisionId) => `후보 ${candidateId}를 결정 ${decisionId}에 연결했습니다. 추적할 예측으로 만들려면 argus_save_prediction으로 저장하세요.`,
      ...
      watch: {
        ...
        captured: (kind) => `기록했습니다 (${kind}). 내부 메모에 남아 있고, 결정의 전제로 추가할지는 당신이 정하면 됩니다 (argus_clarify_decision).`,
