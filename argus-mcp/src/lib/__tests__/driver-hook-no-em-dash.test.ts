 ...
 * argus-driver의 session-start 훅은 별도 패키지에서 사용자에게 relay되는
 * 문구(`lines.push(...)`, `return \`Argus: ...\``)를 직접 만든다 — 여기에
 * 금지된 em-dash cadence가 세 줄 살아 있었다(F12). 드라이버엔 테스트 러너가
 * 없어(순수 플러그인), 같은 모노레포·같은 라이선스 존인 MCP 스위트에서
 * 형제 파일을 읽어 재발을 loud하게 막는다.
 *
...
// argus-mcp/src/lib/__tests__ → 레포 루트 → argus-driver/hooks
