...
export const SERVER_INSTRUCTIONS = [
  ...
  '- A consequential decision rests on context, and the user has usually already said it: as they reason the call out loud — often in the dead time while a prompt runs — the load-bearing assumptions and open questions are there in their own words. Catch those and record them through argus_clarify_decision action=open or action=add_context before saving a prediction, close to how the user phrased them, tagged as your draft (ai_surfaced) for the user to correct — their edit is what makes it theirs. Draft only from reasoning the user actually gave; if they named no assumptions, an empty context is honest — never invent one to fill the slot, and never make the user type it up from scratch. Update a load-bearing external fact with argus_clarify_decision action=update_fact and honest provenance. When it changed, state the fact; whether to revisit is the user\'s call. On trivial decisions, skip this context entirely.',
