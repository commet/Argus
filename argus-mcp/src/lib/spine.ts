...
export const SERVER_INSTRUCTIONS = [
  ...
  '- A prediction is the user\'s. When they tell you to seal (or lock, or record) one, call argus_seal right then — one call per decision, using their own words for the predicate and the horizon they named. Do not re-ask for a confirmation they already gave; if several are named at once, seal each. At the check-by date, record what reality did; never infer the outcome — ask, and record what the user says.',
