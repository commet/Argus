...
- `[RENDER_THIN]` **ForkPath**, **SailingShip**, **BranchMap** — false positives. All
  sparse, textless line/SVG diagrams (SailingShip paints a full 46 KB ship; ForkPath a
  fork diagram; BranchMap a narrow version-tree SVG ≤320px). ForkPath's preview injects a `<style>` that disables
