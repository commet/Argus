...
- `~/.Codex/skills/{name}/SKILL.md` (global)
- `.Codex/skills/{name}/SKILL.md` (project)

### Check 2: Agent files

Verify:
- `~/.Codex/agents/devils-advocate.md` exists
- OR `.Codex/agents/devils-advocate.md` exists (project)
...
```
  ...
  ⚠ Journal has 52 entries (recommended: archive after 50)
    ...
    Fix: Move older entries to .argus/journal-archive.md
...
```
  ✓ All systems healthy. Argus is ready.
