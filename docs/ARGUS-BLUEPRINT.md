...
- **계획 수정 지원 (2026-08-15, 11차 리시트 실측)**: 정당한 소수정("4번에
  롤백 비용 넣어줘")이 PLAN_ALREADY_ADOPTED 정직 거절에 유실된다. 낡은 귀환
  계약의 명시적 폐기 이벤트와 과발화 상호작용 설계가 필요 — 밤에 짓지 않고
  여기 적는다.
- **v2 원장 어휘의 계획 이벤트 확장 (2026-08-15)**: plan_adopt/plan_check가
  v2 거울에 매핑이 없어 계수된 보류로 남는다(v2_write.mirrored:0으로 관측
  가능, v1 원문은 보존). bearing 가족은 날짜 없는 단계라 하중(귀환 연결)을
  못 담는다 — 어휘 확장은 스키마 설계 결정으로 창업자 검토에 올린다.
- **위임 규칙(delegation) 수집의 로컬 이식 (2026-08-15, 14차 전수 대조)**:
  원격 파일럿 뇌의 인지 스키마 중 로컬 MCP에 이식되지 않은 마지막 조각
  (질문·가치·확신도·버린 대안·믿음은 사이클 1~4로 이식·소비까지 완료).
  위임 정책의 성적은 TWIN 수정조항 규율(채점 대상은 정책이지 사람이 아님)을
  따라야 하므로 수집 표면·소비처·과발화 게이트를 함께 설계해야 한다 —
  새 수집 표면이므로 여기 적고 짓지 않는다.

- **`INVALID_INPUT: the request needs checking` — 필드도 이유도 없는 막다른 거절
  (2026-08-11 여정 O1 실측, 원인 미규명).** 한 실행에서 **5번 연속** 나왔고 그
  실행 최다 거절이었다. 어시스턴트는 무엇을 고쳐야 하는지 못 들어 다섯 번
  재시도하고 끝내 못 뚫었으며, 그 실행은 전제 0건으로 끝났다. 휴머나이저의
  `default:` 분기가 루트 경로(`(root)`) 이슈에서 내는 문구인데, **재현에
  실패했다** — 스키마를 여러 모양으로 두드려도 루트 이슈가 안 나왔고,
  트랜스크립트가 인자를 260자에서 잘라 실제 payload를 볼 수 없었다 (그 자름은
  이번에 고쳤다: 거절된 호출은 인자 전문을 남긴다). **추측으로 고치려다
  한 번 실패했다** — `custom` 이슈의 message를 통과시키는 수리를 만들었는데,
  재발신된 custom message가 Zod 원문(`expected string, received undefined`)이라
  휴머나이저가 막으려던 잡음을 오히려 흘리는 회귀였고 되돌렸다. 다음 여정
  실행에서 전문 로그로 payload를 잡은 뒤에 고친다.
...
- **정산이 경쟁 과제에 밀린다 (2026-08-14, 재현·귀속·1차 수리 실패 — 보류)** —
  [9차 리시트: 위치 수리 산문(봉투+도구 설명)은 도달했는데도 무효 (P08 2/3
  잔존). 산문은 행동 억제엔 통하고 행동 생성엔 약하다. **심각도 하향**: 확인일
  check_in이 놓친 정산을 스스로 되살림을 실증 — 손실이 아니라 하루 지연.
  데이터 어포던스류 후보는 근거 약으로 보류, 재개 조건은 새 가설 + 실사용
  증거.]
  [대조쌍 측정(8차 리시트): 하중 축은 Terse(압축)가 아니라 **Directive(다음
  지시 발행)**다. V팔(Terse→Rambling) 놓침 2/3 지속, D팔(Directive→Consensus)
  제품 귀속 놓침 0/3 — D팔의 유일한 놓침은 사용자가 직접 기록을 미룬 것이라
  침묵이 정답. 수리 표적은 "결과 진술 + 다음 지시" 연쇄의 라우팅이며, 사용자
  유예의 경계(D8-2 실례)가 수리 설계의 제약이다. 사전등록 예측(Terse 주동력,
  p=0.55)은 빗나감으로 정산.]
  페르소나 스윕 T08(P08 한국어·Blunt/Terse/Directive)에서 사용자가 결과를
  명시하며("완전 빗나감") 디버깅 부탁을 같은 발화에 붙이자, 어시스턴트가
  정산을 건너뛰고 디버깅으로 직행했다. P03·P05·P10은 같은 구조에서 전부
  정산했다. **재현 측정(2026-08-14, U8 ×3): 4회 중 3회 건너뜀 vs 다른
  페르소나 6회 전부 정산** (Fisher 단측 p≈0.03, 소표본). 봉인은 전부
  존재했고, U8-1 어시스턴트의 발화가 메커니즘을 그대로 보여준다: "이건
  디버깅/분석 작업 요청이라 Argus에 기록할 결정 사항은 없어 보이네요" —
  결과 진술 뒤에 다음 과제가 붙으면 기록 대상이 아니라고 판정한다. 축이
  행동을 바꾼 확인된 실측이며, 수리는 별도 설계 후에만 연다 — 후보는 도구
  표면이 아니라 침묵 계약과 겨루지 않는 자리(6차 리시트 참조). 이 항목이
  축별 조작 점검 계측기의 첫 양성 데이터다.
- **정산 `id`의 비대칭 (2026-08-12, 짓지 않음 — 근거가 인공물로 무효화됨)** —
  도구 표면에서 id를 설명하는 유일한 문장은 봉인 쪽의 *"A short slug you
  **pick**"*이고, 정산 쪽 `id`에는 설명이 없다. 즉 표면은 "골라라"만 가르치고
  "찾아 써라"를 어디서도 정정하지 않는다. 여정 실측에서 정산 첫 시도가 3/3으로
  술어에서 id를 지어냈기에 이걸 원인으로 의심했으나, **그 측정은 하네스가
  `data.id`와 자기 인자를 모두 버린 상태에서 나온 것**이라 근거가 되지 않는다
  (4차 리시트). 채널을 고친 뒤 같은 증상이 재현되면 그때 연다. 비용은 도구
  표면 예산(헤드룸 217B)이고, 새 문장 없이 런타임에서 이미 `saved_ids`가
  1왕복에 복구시킨다 — 즉 열더라도 표면이 아니라 런타임이 먼저다.
...
| `ARGUS-REFOUNDATION-BRIEF-2026-08-16.md` | 방향 정본 — **창업자 §6 결정 3장 봉인(2026-08-18)** | 재정초 — 문제 3층·불변 속성 P1~P8·원형 카탈로그·판별 프로그램. §5 실측(E-0 + B/E/G)은 완료. §6 결정: ① 고객 = **사람**(에이전트는 실행 도구·규율의 시험장) ② 무게중심 = **동반자(대화 안)** — 1차 표면은 MCP 도구·플러그인 훅, 웹은 정산·거울 ③ PR #396 자산 = **엔진 편입 + 화면 철거** (순수 엔진 `src/lib/cognition/`은 라이브러리로 존치해 대화 루프에 편입, 링크 없는 파일럿 화면은 철거, 리시트는 정산된 기록으로 보존). 이 봉인 전의 원형 시공(PR #396 파일럿 화면)은 §5 수칙 위반으로 기록하고 되돌렸다 |

새 문서는 이 표에 등록하지 않으면 §8 대기 목록 규칙 위반이다.

**재정초 §6 봉인 (2026-08-18, 창업자 결정 — 이 절의 최신 방향).** 위 표의
`ARGUS-REFOUNDATION-BRIEF-2026-08-16` 참조. 결정 셋이 봉인됐다: 고객은
사람(층③ 저자성 혼입이 핵심 병, 에이전트는 실행 도구), 무게중심은 동반자
(대화 안 — 오염이 일어나는 자리에서 마찰 0에 수렴, M7이 형태를 정한다),
PR #396 자산은 엔진 편입 + 화면 철거. 여기서 도출되는 다음 시공 순서:
(1) 이 등록 (앱 존, PR #396에서) → (2) MCP `argus_predict` 봉인 흐름에
전제 1개 + 감시 신호 원탭 결박 — `src/lib/cognition/watch.ts`의 사람말
질문 4개를 그 흐름에 이식 (**MIT 존, 별도 PR**) → (3) 전제 흔들림의 귀환은
기존 T2 게이트로 (새 알림 유형 금지 규약 유지) → (4) 창업자 실결정 1건으로
봉인→감시→귀환→정산 실주행 1사이클 + 원장 SQL 대조 (HANDOFF "아직 안 돈 것"
2번) → (5) 각 단계에서 M7 실측 후에만 다음 승격. 절차 기록: PR #396의 커밋
8개가 존재하지 않는 "공정 12"를 첫 줄에 달았다 — R 트랙 작업은 위 표의
PRODUCT-PLAN §6이 순서 정본이고 §6 공정표(0~5)는 기존 제품 유지보수 전용이므로,
그 표기는 무효다. 이력은 다시 쓰지 않고 여기 남긴다.
