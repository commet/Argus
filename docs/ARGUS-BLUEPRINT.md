...
- exit: [ ] docs/ 최상위 정본 8편 [ ] CLAUDE.md에 BLUEPRINT 포인터
  [ ] /admin에 표면별 4단 퍼널 [ ] 내비에 항구·워크스페이스·설정만
...
- exit: [ ] 여정 fixture CI 초록 [ ] ko 영수증 전문 한국어 스크린샷
  [ ] missed가 웹에서 missed로 (SQL 대조) [ ] 만료 토큰 seal의 surface에
  실패 1줄 [ ] 이메일 CTA 1클릭=해당 결정 4-tap (실메일 캡처)
...
- 작업: §4.1 헌법을 notification-gate.ts로 · T1 문안 시공 · T4 1차 정산
  ...
  템플릿 (email-html.ts).
- exit: [ ] gate reflection 테스트(6 cron 전부 통과) [ ] 30초 계약 테스트
  [ ] 신규 사용자 fixture의 첫 귀환 ≤ 7일 (T1 또는 T4) [ ] 빈 브리프 0건
  테스트 [ ] 정산 화면 Playwright 스냅샷(라이트/다크)
...
- 작업: T2 문안 시공 + materiality 판정과 이메일/브리프 강등 라우팅 ·
  ...
  어휘와 웹 문안 통일.
- exit: [ ] 전제 드리프트 fixture: 감지→gate→이메일 페이로드→전제 화면
  딥링크 여정 테스트 [ ] 해석 어휘 validator (알림 문안에 권고/평가 표현이
  들어가면 테스트 실패)
...
- exit: [ ] 공유 링크 OG 카드 렌더 캡처 [ ] 기본 비공개 테스트
  [ ] 데모 시나리오 전부 확인일 ≤ 14일
