...
- **M5 보정 거울의 진짜 선행 조건 = 사용자 확신도 사전등록 입력 (2026-08-06).**
  TWIN §4.4가 약속한 "도메인별 Brier 추이" 중 사용자 쪽 절반은 사용자가 확신도를
  **미리** 적는 입력이 없으면 계산할 수 없다. 없는 입력으로 낸 보정 점수는 이
  리포가 금지한 그럴듯한 가짜 숫자다. 그 입력을 어느 표면에 넣을지가 도구 6개
  봉인과 부딪히므로 창업자 판단이 선행한다. 자세한 것은 TWIN 기획서 부록 C.5.
