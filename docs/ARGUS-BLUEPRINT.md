...
- **정산이 경쟁 과제에 밀린다 (2026-08-14, 짓지 않음 — n=1 재현 대기)** —
  페르소나 스윕 T08(P08 한국어·Blunt/Terse/Directive)에서 사용자가 결과를
  명시하며("완전 빗나감") 디버깅 부탁을 같은 발화에 붙이자, 어시스턴트가
  정산을 "이미 빗나간 걸로 확인됨 — 새 예측 불필요"라며 건너뛰고 디버깅으로
  직행했다 (resolve/predict 혼동). P03·P05·P10은 같은 구조에서 전부 정산했다.
  n=1이므로 Terse×Directive 재현 측정(≥3회) 후에만 연다. 열 경우 후보는 도구
  표면이 아니라 침묵 계약과 겨루지 않는 자리 — 5차 리시트 참조.
