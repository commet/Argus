...
| 9 | Self-honesty (no unearned track record) | **7.5** (was 6.5) | enforceable invariant (calibration-disclosure.ts) + WIRED to the real record: `recordDisclosure(CrossProjectRecord)` gates on settled `loops`, so every track-record surface reads showStats/banner from the lib (no per-UI edit, no collision) — 9/9 tests | the UI components still need to call `recordDisclosure` to render the banner (lib is ready; adoption is a 1-line consumer change each) |
...
**Average ≈ 8.4** (follow-ons landed: dim8 seal provenance, dim2 crisis-taxonomy under the generator, dim9 wired to the real record). Honest verdict: **not "10 tens"** — but every dimension is now genuinely-implemented + validated, with the residuals being UI-adoption 1-liners gated on the active North-Star/BIND sessions. Five dimensions are genuinely
