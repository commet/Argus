...
|---|---|---|---|---|
| 1 | Behavioral eval | **9.5** (was 9) | clarify+sail fidelity, **corpus 25→50** (flat/fork/crisis/low/sourced/inject, validated by the static gate), judge retry→inconclusive, **per-tier sweep tooling** (sweep.mjs + EVAL_KINDS filter — runs the over-fire-sensitive cases across haiku/sonnet/opus and gates the spine floor on EVERY tier) | the live 50-case run + the tier-sweep NUMBERS are pending an API-credit top-up (the session's key ran dry); the harness itself ran live earlier and is proven — only the fresh numbers wait |
