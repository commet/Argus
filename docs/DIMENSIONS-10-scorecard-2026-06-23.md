...
| 8 | Seal-time engine provenance | **5.5** | capture half: api/llm now logs model id + token usage (was discarded) — the capture-now-or-lose signal | the seal-time stamp on the contract is NOT done — SealMoment/decision-contract.ts are actively owned by the North-Star/BIND session; editing them = the collision I was told to avoid |
...
**Average ≈ 7.8.** Honest verdict: **not "10 tens."** Five dimensions are genuinely
