...
| **F. 패키징이 publish 불가 — `npx -y argus-decision-mcp`가 fresh install서 튕김** | `package.json`에 `bin → dist/index.js`는 있으나 `prepublishOnly`/`prepare` 빌드 없음, `files` 화이트리스트 없음, `dist/` 미커밋, `engines.node` 없음, **테스트 러너 devDep 자체가 없음**(vitest/jest 0). §5.4 헤드라인 `claude mcp add argus -- npx -y argus-decision-mcp`가 첫인상인데 현재 바운스. | `"engines":{"node":">=18"}`, `"files":["dist"]`, `"prepublishOnly":"npm run build && npm test"`, `"typecheck":"tsc --noEmit"`, `"test":"vitest run"` 추가 + vitest devDep + `dist/` 커밋 or prepublishOnly 의존. **A3(Windows npx cold-start 5~15s) 주석**: §5.4에 "첫 시작 타임아웃 시 `npx -y argus-decision-mcp --version` 1회로 캐시 워밍" 추가. | §5.3, §5.4 |
