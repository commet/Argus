...
> **엔진은 세계 최상급으로 잘 만들었는데, 시동이 단 한 번도 걸린 적이 없다.**

엔지니어링 장인정신(LLM 플루밍, 결정 계약 코어, 가드 테스트, 웹훅 보안)과 디자인 장인정신(랜딩)은 진짜다 — 흉내가 아니다. 그런데 제품의 존재 이유인 핵심 루프(seal→settle, 결정을 봉인하고 나중에 현실과 정산)는 **창업자 본인 포함 그 누구도 단 한 번도 끝까지 돌린 적이 없다**(라이브 DB: `decision_graded` 이벤트 0건, 영속된 `decision_contract` 0건). 나머지 모든 것(메타포, 17 에이전트, 3D 항해 영상)은 아직 시동도 안 걸린 자동차를 위한 마케팅이다.
...
**C1. 핵심 루프가 한 번도 닫힌 적 없다 — 계측 아티팩트가 아니라 진짜다.**
라이브 DB: `decision_graded` 이벤트 *통산 0건*, 영속 `decision_contract` *0건*, `plugin_decisions`/`plugin_bearings` 0건. 제품의 존재 이유(나의 n=1 정산된 트랙레코드를 소유)가 ~3개월/47 프로젝트 후 뒷받침 행이 0이다. (`argus_metrics`, 라이브 쿼리)

**C2. 해자(moat)가 "미검증"이 아니라 자기 데이터로 "반증"됐다.**
방어가능성 전부가 n=1 decide→predict→settle 누적에 걸려 있는데, 퍼널은 47 오픈 / 0 봉인 / 0 정산(창업자 포함). 이탈은 깨진 정문이 아니라(문은 47번 열렸다) **정확히 해자가 살아야 할 그 지점에서** 일어난다. 상상 가능한 가장 동기 높은 코호트(창업자가 직접 고른 초기 사용자)가 핵심 루프를 0% 완주했다. 가장 동기 높은 사용자도 한 번 안 지나는 해자는 해자가 아니다.
...
**H1. (가장 큰 발견) 창업자 dogfood 처방이 부비트랩에 걸려 있다 — 두 렌즈가 따로 봐서 아무도 연결 못 함.**
- GTM 렌즈 처방: "창업자가 진짜 결정 3개 seal + 1개 settle 해서 시동을 걸어라."
- 백엔드 렌즈 발견: return-trigger 리마인더(`checkin-due`)가 **Resend 샌드박스 발신자 `onboarding@resend.dev`에서 보냄.** Resend 규칙상 이 주소는 **계정 소유자 본인의 인증 주소로만** 배달된다.
- **합치면:** 이 버그는 지구상 정확히 한 사람 — 창업자 — 에게만 작동하고 실사용자에겐 *조용히 전부 실패*한다. 그래서 "성공적인" 창업자 dogfood는 **거짓 양성 검증**을 만든다: 창업자는 리마인더를 받고 settle하고 0→1을 보고 "루프 작동한다" 결론 내리지만, 인프라는 구조적으로 다른 누구에게도 그 리마인더를 못 보낸다(익명 seal은 서버에 아예 안 닿음). 테스트하라고 지정된 그 한 코호트가, 깨진 파이프가 안 깨지는 유일한 코호트다.
- **수정:** `checkin-due/route.ts:99`의 발신자를 인증된 `argus.voyage` 도메인으로 바꾸고(=`email/send`처럼), **비창업자 주소로 실제 리마인더 1통 보내 배달 확인 후에** cron을 믿어라. (1줄 수정, 전체 surface에서 최고 레버리지)
