...
- 각 단계: 항로 재설정 → 선원 배치 → 리허설 → 항로 수정 → 결과
- 선원 배치 섹션: **actor 토글 인터랙션** (AI/사람/협업 클릭 전환, 실시간 통계 변동)
...
- **ProcessSteps**: 항로 수정(4단계) 항상 표시 + getStatus 케이스 추가
- **모바일 탭바**: 4탭 (해석·선원 배치·리허설·종합), 터치 타겟 44px 보장
...
- **전제 조건/팀 리뷰/상황·문제·접근 스토리라인**: 제거 (항로 재설정에서 이미 다룸)
...
- **빈 상태(Empty State)**: 항로 수정, 페르소나, 프로젝트 — 행동 유도 CTA 추가
...
| **1** 적응형 항로 재설정 | 완료 | `src/lib/reframing-strategy.ts`, `src/lib/eval-engine.ts` |
| **2** 다관점 선원 배치 검증 | 완료 | `src/lib/workflow-review.ts` |
...
5. **팀 리뷰**: 선원 배치에서 제거됨 — 별도 단계 또는 리허설에서 재도입 필요
6. **선원 배치 "직접 입력" UX**: 칩+텍스트 병행 구현했으나 실사용 검증 필요
...
### 즉시
1. **전체 흐름 E2E 테스트** — 항로 재설정 → 선원 배치 → 리허설 → 종합 전 과정 실사용 확인
2. **선원 배치 스텝 카드 UX 검증** — vs 칩 파싱 동작 확인, 클릭 반응성
3. **리허설 + 종합 화면 디자인 검토** — 선원 배치과 동일 수준으로 개선 필요

### 단기
4. **Supabase 마이그레이션** — 모든 새 필드 DB에 반영
5. **모바일 반응형 검증** — 좌우 레인, 데모, 랜딩 페이지
6. **팀 리뷰 재배치** — 선원 배치이 아닌 적절한 단계에서 재도입
...
3. **선원 배치 카드 collapse 기본**: 빈 입력 필드 12개가 한꺼번에 보이면 포기. 기본=간결, 클릭=상세.
...
6. **전제 조건/팀 리뷰 선원 배치에서 제거**: 전제는 항로 재설정에서 이미 다룸. 팀 리뷰는 선원 배치 단계에서 하는 게 부자연스러움 — 리허설 이후 또는 별도 단계로 재배치 예정.
...
9. **실행 준비도 = 과정 평가**: 핵심 4단계 각 20점, 리스크+수렴 각 10점. 항로 수정 없이도 80점(실행 준비 완료) 가능.
