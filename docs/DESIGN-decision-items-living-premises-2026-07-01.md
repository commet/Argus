...
|---|---|---|
| `off` | 감시 안 함 | **대부분 항목** |
| `on_change` | 주기적 web 재확인, **바뀌었을 때만 알림** | external + load_bearing 전제 |
| `weekly` / `monthly` | 정해진 주기로 재고 유도 | 사용자가 켤 때만 |

### 5.2 자제 규칙 (알림이 잔소리가 되지 않게)

- 기본은 **거의 다 off**. 켜지는 것은 사용자가 중요로 표시한 external 전제뿐.
- 그것도 `on_change`(현실이 실제로 바뀔 때만) 이 기본. **정해진 주기 나그냄은 opt-in.**
- SessionStart의 침묵-기본, helm의 "하중+비가역에만 발화"를 그대로 계승.
- **drift 판정은 기계적으로**(공유 `premise-drift.ts`), 알림 발화 여부는 LLM 재량이 아니다.
...
7. **자제(mirror clause)** — §5.2. 알림 기본 off, on_change는 현실-게이트.
