...
## W5-1 — P1-D2: 06 S1-6 부제 교체 미실행

- **무엇이 애매했나**: 출처 스펙(06 S1)은 6개 항목인데 마스터 §2 P1-D2 문면(정본)은 4곳(카드·타임라인·manifest·갈래 칩)만 명시. S1-6(CurrentBearingCard 부제를 "…한 장으로 요약한 거예요" 문안으로 교체)을 할지.
- **창업자라면**: 마스터가 정본이고("위 요약과 다르면 00-MASTER.md가 이긴다"), 현행 부제("이 결정이 지금 향하는 방향입니다. 긴 문서는 아래 근거로 접어두고, 나중에는 여기서 다시 이어갑니다.")는 제안 문안과 취지·정보가 사실상 동일 — 이미 개선된 상태로 판단했을 것. 최소 diff 원칙.
- **내린 판단**: 정본 4곳만 개명, 부제 무접촉.
- **되돌리는 법**: CurrentBearingCard.tsx:127-131의 L() 문안을 06 S1-6 제안문으로 교체하면 끝(독립 1줄 변경).

## W5-2 — P1-D7: NextStepGuide 링크 유지 + 레벨/XP 블록 무접촉

- **무엇이 애매했나**: 마스터 문면은 "NextStepGuide 링크 교체(레거시 프로젝트 안에서만 허용)"와 "레벨/XP 문단을 항해 4박자로 교체"인데, 실코드 확인 결과 ① NextStepGuide는 레거시 스텝 컴포넌트(Reframe/Recast/RehearseStep) 안에서만 렌더 ② 레벨/XP 블록은 감사 이후 누군가 이미 접힌 details로 강등해 둠.
- **창업자라면**: 스펙 자신이 조건을 달아뒀다(05 S3: "레거시 전용이면 그대로 두는 것도 가능. 원칙: 레거시 안에서는 레거시 링크 허용" / 05 S4: "삭제하거나 접힘 강등"). 조건 충족 = 손대지 않는 게 스펙 준수. "항해 4박자로 교체"의 실체는 가이드의 주 가르침(quickStartSteps)을 현행 플로우로 바꾸는 것.
- **내린 판단**: LegacyChip 블록 삭제 + quickStartSteps 4박자 교체만 실행, NextStepGuide·레벨 details 무접촉.
- **되돌리는 법**: NextStepGuide.tsx의 /tools/recast·/tools/rehearse href를 /workspace로 바꾸면 원 문면 그대로(3곳). 레벨 details 삭제도 독립 블록이라 기계적.

## W5-3 — MCP 소품 중 "목소리분"의 경계

- **무엇이 애매했나**: 웨이브5 정의는 "MCP 소품 중 목소리분(REASON_LINE 등)" — "등"의 범위가 미명시. MCP 소품 묶음은 10건.
- **창업자라면**: 웨이브5는 "카피만"의 웨이브(§3: "목소리 대청소 — 카피만"). 사용자에게 보이는 문장을 바꾸는 것(REASON_LINE enum→사람 문장, dropped_lines 침묵→발화)이 목소리분이고, 애노테이션·버전·README 표·폴백·wrap 같은 구조/정합 소품은 웨이브8 "MCP 소품 잔여"에 명시적 자리가 있다.
- **내린 판단**: REASON_LINE + dropped_lines 고지 2건만 이번 웨이브. 나머지 8건은 웨이브8로.
- **되돌리는 법**: 해당 없음(추가만 했고 잔여는 미룸) — 웨이브8 에이전트는 11 S7·12 §3.6 목록에서 이 2건을 빼고 실행하면 됨.

## W5-4 — P1-D3 스모크의 측정 대상 정의

- **무엇이 애매했나**: "30초 안팎이면 첫 읽기가 와요"의 "와요"가 첫 토큰인지 완주인지. 텔레메트리는 측정 불가(llm_usage에 duration 없음)라 프로덕션 스모크로 대체 — 익명 무료 쿼터 3회 소모.
- **창업자라면**: 사용자 체감 기준은 "읽기 결과를 손에 쥔 순간" = 완주. 앱 안에 이미 "보통 20~40초"(workspace 로딩 카피)라는 동일 약속이 있어 정합.
- **내린 판단**: 완주 기준(실측 16~30초)으로 "30초 안팎" 유지. 첫 토큰(~2.8초)은 보너스 근거. 측정치를 코드 주석에 박제해 다음 사람이 재검증 가능하게 함.
- **되돌리는 법**: SirenHero의 해당 L() 한 줄에서 시간절만 빼면 됨("첫 읽기가 바로 시작돼요").

## W6-1 — 재봉인(서랍 "이대로 다시 약속")에는 의식 재생 안 함

- **무엇이 애매했나**: §4 채택 조건 "모든 봉인에 동일 1회"에서, 봉인 직후 손보기 서랍으로 날짜·예측을 고쳐 다시 봉인하는 경우에도 2.6초 의식을 다시 재생해야 하는가. 07 S3은 seal()이 'sealing' 진입이라고만 적음.
- **창업자라면**: "동일 1회"의 취지는 결정의 내용·방향에 따라 의식이 달라지지 않는다는 무차별성이지, 조정 때마다 반복 재생하라는 게 아니다. 절제 기본값(over-fire 거울 조항) — 방금 의식을 본 사용자에게 같은 의식을 또 트는 건 ceremony 남발.
- **내린 판단**: 의식은 세션당 1회(ask→sealing 경로만). 증서 위 서랍에서의 재봉인은 증서에 조용히 머무름. 코드 주석으로 박제.
- **되돌리는 법**: SealMoment.seal()의 `setScene((s) => s === 'sealed' ? 'sealed' : …)` 한 줄을 무조건 'sealing'으로.

## W6-2 — 온램프 "다음 확인할 것 N건"의 동작 = 모달 닫기

- **무엇이 애매했나**: 마스터는 "남은 due 있으면 '다음 확인할 것 N건'" 손잡이라고만 함 — 목적지 미명시. SettlementModal은 /project에서만 마운트되므로(전수 grep) /project 링크는 제자리 링크.
- **창업자라면**: 귀환의 모이는 항구는 /project 하나(§5-11)고 due 스트립이 모달 바로 뒤에 있다 — 문을 보여주는 것이지 열어젖히는 게 아니다(§4 온램프 계약).
- **내린 판단**: 링크 동작 = onClose(모달 닫기 → 스트립이 바로 보임). N은 부모의 useDueCount를 prop으로 내려받아(자체 산수 금지) 스트립 숫자와 드리프트 불가. due 0이면 "새 결정 적기 → /workspace" LocaleLink. remainingDue prop 부재(레거시 마운트·테스트)면 새-결정 문이 기본 — 둘 다 사실 문이라 안전.
- **되돌리는 법**: SettlementModal의 onClose 버튼을 `<LocaleLink href="/project">`로 교체하면 이동형.

## W6-3 — preview 스크린샷 불가 → eval 실기동 검증으로 갈음

- **무엇이 애매했나**: 웨이브 경계 검증의 "preview 스크린샷 시도" — dev 서버는 뜨나 preview_screenshot이 30s 타임아웃 반복(과거 세션 확정 환경 한계).
- **내린 판단**: 태그의 명시 대체 경로(클래스·reduced-motion 단위 테스트) + 추가로 eval 실기동 2건(서빙된 CSS의 seal-* 5종 computed style 확인, 판단 액자 실DOM 렌더 확인)으로 갈음. 봉인 증서·의식 장면 자체의 픽셀 확인은 실제 항해 완주가 필요해 아침 창업자 육안 확인 항목.
- **되돌리는 법**: 해당 없음(검증 방식 선택).

## W7-1 — 3고리 문턱의 카운트 = 병합 정산 수 (프로젝트 loops + 검수 settled)

- **무엇이 애매했나**: 08 S5 문면은 "`loops`가 정확히 SETTLED_THRESHOLD(3)에 처음 도달하면"이라 프로젝트 고리만 셀 수도 있음. 그런데 같은 웨이브의 A2가 자차표의 dim9 게이트를 **병합** 정산 수(프로젝트+검수)에 적용하도록 바꿨다 — 이탤릭이 사라지는 문턱과 의식이 울리는 문턱이 다른 수를 세면 두 표면이 어긋난다(이탤릭은 3에서 사라졌는데 의식은 아직, 또는 그 반대).
- **창업자라면**: 웨이브 정의 자체가 "P1-A5 (RecordStrip 카운트 필요)"라고 의존을 명시 — A5는 A2의 카운트 소스를 먹으라는 뜻. 기록이 하나라고 말해놓고(한 뇌) 의식만 딴 장부를 세는 건 자기모순.
- **내린 판단**: `shouldShowThirdLoop`는 병합 수(=RecordStrip 게이트와 같은 수)로 판정. 엄격 동치(===3)라 이미 지난 사용자에게 소급 발화도 없음.
- **되돌리는 법**: SettlementModal의 merged 계산에서 `summarizeReviewRecord(receipts).settled` 항을 빼면 프로젝트-only.

## W7-2 — 텔레그램 어휘 통일 범위 = 기록 요약 줄만 (답변 버튼 무접촉)

- **무엇이 애매했나**: 08 S2-5 "어휘만 통일('잘됨/안됨' → '적중/빗나감' 계열)"의 범위 — record-core의 기록 요약 줄 외에 telegram-settlement의 정산 답변 버튼("✅ 잘 됐어요 / ✋ 안 됐어요")까지 바꿀 것인가.
- **창업자라면**: 버튼은 "그래서, 어떻게 됐어요?"에 대한 사용자의 **대답**(현실 서술)이고 W2가 seal-core 한 뇌로 이미 맞춘 정산 질문 카피의 일부 — 기록 요약은 쌓인 **장부**의 어휘. 두 자리는 역할이 다르고, S2-5가 지목한 파일도 record-core.ts뿐.
- **내린 판단**: record-core.ts의 정산 결과 줄만 "적중/빗나감"으로. 버튼·파서(happened/avoided 콜백)는 무접촉 — 파서는 어차피 enum이라 의미 불변.
- **되돌리는 법**: record-core.ts의 해당 두 줄 문자열만 원복(테스트 단언 '적중 3'도 함께).

## W7-3 — renderWake 시그니처에 recordSince 5번째 인자 추가
