...
Argus helps a person orient inside important decisions without losing the trail of why.

It is not a multi-agent showcase, a generic risk machine, or a prettier ChatGPT
answer. The product is a decision-orientation loop: clarify the real question,
do the necessary agent work, preserve real alternatives, verify claims, choose
the current course, and leave behind an auditable log that can later be checked
against reality.
...
> "I know where this decision currently stands, why that course is justified,
> what remains unresolved, what path I am not taking, and what must happen next."
...
Argus is a decision-orientation harness for judgment.

- **The user is the captain.** Argus never pretends to decide for them.
- **Agents are workers, not the product.** They do work in the background, but the
  user should not have to admire the machinery.
- **The decision trail is the artifact.** The process matters because it records how the
...
- **Verification is a compass, not a scoreboard.** The point is not to produce a
  ...
  course.
- **The decision contract closes the loop.** When a decision is sealed, it
...
The user should see a **Current Course**:
...
Set-aside option: full consolidation now. It would spend migration cost before proving surface demand.

Next step: pull DAU split by surface, then run the spike.
...
The webapp owns the richer decision-trail experience:

- visual progress through origin, briefing, worker output, review, mix, and seal,
- branch/fork/switch/seal interactions,
...
- reads files, PRs, branches, repo state, and local documents from the user's
  actual workspace,
- runs agent work where the user already works,
...
- returns one Current Course instead of a workflow transcript.

They do not need identical UI. They need identical product truth: Argus is a
decision trail with memory, verification, forks, and eventual reality checks.
...
2. Use agents as workers on the actual artifact or decision.
3. Verify supported, challenged, unresolved, and human-required claims.
4. Optionally run stakeholder review.
5. Render a Current Course.
6. Preserve the full decision trail in `.argus/sessions/`.
...
- it gives a recommendation without a set-aside option,
...
- **Current Course**: the one-screen default output.
- **Open Risk**: the one unresolved point that could change the course.
- **Set-aside Option**: an alternative consciously not taken now.
- **Next Step**: the smallest concrete action from here.
- **Prediction to Check**: a falsifiable future predicate.
- **Decision Trail**: the durable record of what happened and why.
- **Worker Agents**: agents doing real work in the background.
- **Navigator**: the synthesizer that turns agent work into a course.
- **Fork / Branch**: an alternate course.
- **Decision Contract**: a later-checkable prediction bundle.

Use these sparingly:

- **Voyage**, **crew**, **anchor**, **sail**, **helm** can remain as command names,
  legacy aliases, or light internal flavor. Do not stack them in default output.
