...
이 절은 인수 당시의 과거 상태를 보존한다. 당시에는 코드와 로컬 전수 검증이
끝났고, PR CI, main 병합, 태그/배포, published tarball 재검증이 남아 있었다.
현재 완료 상태는 문서 끝의 13절을 기준으로 판단한다.

## 0. 가장 먼저 읽을 결론

> **과거 상태 주의:** 이 절은 인수 직전의 빨간 기준선을 설명한다.
> 현재 상태가 아니며, 최종 판단은 13절을 우선한다.
...
인수 당시 상태는 이 정의상 **미완료**였다. 현재 완료 증거는 13절에 기록한다.
...
위의 `2.0.8/3.0.8` 복구선은 더 이상 계획이 아니라 배포 완료 상태다.

- 최종 제품 코드가 포함된 태그 `v2.0.8`은
  `e042672661dc9bf2379a5d1b19f61a30364321b8`을 가리킨다.
- 현재 `origin/main`은 설치 검증의 작업 디렉터리 결함까지 보완한
  `d08e03961a1eb850f6319331b99e5ea8560cc99f`이다.
- npm의 `latest`는 `argus-decision-mcp@2.0.8`이며, 공개 tarball의
  `package/dist/index.js` 실행 권한은 `0755`다.
- Claude plugin의 최종 버전은 `3.0.8`이다.
- GitHub Actions 배포 실행 `30394577937`은 npm 확인, 공개 플러그인 설치,
  설치된 MCP 직접 호출, GitHub OIDC 인증, 공식 MCP Registry publish까지
  모두 성공했다.
- 공개 npm 산출물을 새 디렉터리에 설치한 `verify-published.mjs 2.0.8`은
  버전, 실행 권한, 릴리스 표식, 실제 picker, 빈 schema, 단 한 번의 Accept
  기록과 ledger 재조회까지 통과했다.
- `install-smoke.mjs --published`는 Claude의 실제 격리 환경에서 설치,
  inventory, MCP 연결, 설치된 명령 직접 호출, 비활성화, 재활성화,
  업데이트, 삭제 전 수명주기를 통과했다.
- 최종 배포 workflow가 성공했으므로 공식 Registry publish 요청 자체는
  완료됐다. Registry 공개 조회 API는 이 문서를 마감하는 시점에 간헐적으로
  응답이 지연됐으므로, 조회 응답을 배포 성공의 유일한 증거로 과장하지 않는다.

### 최종 판단

이번 정리는 기능 수를 무작정 줄인 작업이 아니다. 호출 의미가 겹치거나
호스트별 추측에 의존하는 경로를 제거하고, 남긴 경로마다 실제 호스트,
공개 패키지, 설치 수명주기, 장시간 사용자 응답, 변이 테스트라는 독립된
증거를 붙인 작업이다.

특히 응답 시간이 짧다는 이유로 `decline`을 전송 실패로 재해석하는 안은
겉보기 성공률을 높이지만 사용자의 명시적 결정을 훼손한다. 따라서 이를
도입한 충돌 PR은 닫고, 프로토콜의 명시적 결과를 그대로 보존하는 구조를
최종선으로 선택했다.

이후 세션은 과거 버전이나 닫힌 PR의 코드를 기준으로 되돌리지 말고,
`origin/main`과 이 절의 공개 산출물 검증 결과를 출발점으로 삼아야 한다.
